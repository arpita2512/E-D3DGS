...
def scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations, 
    ...
    for iteration in range(first_iter, final_iter+1):             
        ...
        if iteration > lpips_start_iter:
          ...
          lip_loss = 0.01 * ( l1_loss(lip_imgs[0], gt_lip_imgs[0], keepdim=True).mean() + l1_loss(lip_imgs[1], gt_lip_imgs[1], keepdim=True).mean() ) / 2 + \
          0.0005 * ( lpips_criterion(lip_imgs[0], gt_lip_imgs[0]).mean() + lpips_criterion(lip_imgs[1], gt_lip_imgs[1]).mean() ) / 2
          loss += lip_loss
          wandb.log({
              'lpips lips': lip_loss,
          })
