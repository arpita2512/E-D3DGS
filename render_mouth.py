...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, hyperparam=None):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        mouth_mask = torch.as_tensor(view.talking_dict['mouth_mask']).cuda()
        ...
        if name in ["train", "test"]:
            ...
            gt_image_white = gt * mouth_mask + background[:, None, None] * ~mouth_mask
            
            torchvision.utils.save_image(gt_image_white, os.path.join(gts_path, view.image_name))
            gt_list.append(gt)
        count +=1
...
def render_sets(dataset : ModelParams, hyperparam, opt, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, skip_video: bool):
    with torch.no_grad():
        gaussians = GaussianModel(dataset.sh_degree, hyperparam, deform_type="mouth")
