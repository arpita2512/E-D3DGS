...
def scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations, 
    ...
    for iteration in range(first_iter, final_iter+1):             
        ...
        #if iteration > lpips_start_iter:
        #  loss += 0.005 * ((lpips_criterion(lip_imgs[0], gt_lip_imgs[0]) + lpips_criterion(lip_imgs[1], gt_lip_imgs[1])) / 2.0).mean()
        #  wandb.log({
        #      'lip_loss': 0.005 * ((lpips_criterion(lip_imgs[0], gt_lip_imgs[0]) + lpips_criterion(lip_imgs[1], gt_lip_imgs[1])) / 2.0).mean(),
        #  })
